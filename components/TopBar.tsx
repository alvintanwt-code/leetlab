export function TopBar() {
  return (
    <header className="flex h-12 shrink-0 items-center justify-between border-b border-[var(--color-hairline)] bg-[var(--color-canvas)] px-5">
      <div className="flex items-center gap-2">
        <span className="flex h-[26px] w-[26px] shrink-0 items-center justify-center rounded-md bg-[var(--color-primary)]">
          {/* eslint-disable-next-line @next/next/no-img-element */}
          <img
            src="/leet-logo.png"
            alt=""
            width={22}
            height={22}
            className="block h-[22px] w-[22px] object-contain"
          />
        </span>
        <span className="t-body-md font-semibold tracking-tight text-[var(--color-ink)]">
          lab.leet
        </span>
        <span className="ml-1 rounded bg-[var(--color-canvas-soft)] px-1.5 py-0.5 t-micro-cap">
          desk
        </span>
      </div>
    </header>
  );
}
