...
import { hsbcAdapter } from "../lib/scrapers/hsbc";
import {
  getProviderId,
  upsertFund,
  upsertSnapshot,
...
async function main() {
  ...
  console.log(`▸ HSBC scrape · ${sample ? `sample of ${sample}` : "all available"}`);
  const ids = await hsbcAdapter.listFundIds();
  ...
  for (const [i, id] of targetIds.entries()) {
    try {
      const { fund, snapshot, allocations, documents } = await hsbcAdapter.scrapeFund(id);
