...
import { tmlsAdapter } from "../lib/scrapers/tmls";
import {
  getProviderId,
  upsertFund,
  upsertSnapshot,
...
async function main() {
  ...
  const providerId = await getProviderId("tmls");

  console.log(`▸ Tokio Marine scrape · ${sample ? `sample of ${sample}` : "all available"}`);
  const ids = await tmlsAdapter.listFundIds();
  console.log(`  ${ids.length} IDs discovered from list page`);
  ...
  for (const [i, id] of targetIds.entries()) {
    try {
      const { fund, snapshot, allocations, documents } = await tmlsAdapter.scrapeFund(id);
